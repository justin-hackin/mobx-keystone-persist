...
var mobxKeystone = require('mobx-keystone');
  ...
  try {
    ...
    var persistSnapshot = function persistSnapshot(_snapshot) {
      ...
      Object.keys(snapshot).forEach(function (key) {
        if (key === mobxKeystone.modelTypeKey || key === mobxKeystone.modelIdKey) {
          return;
        }
    ...
    mobxKeystone.onSnapshot(store, typeof throttle === "number" ? debounce(persistSnapshot, throttle) : persistSnapshot);
    return Promise.resolve(storage.getItem(name)).then(function (data) {
      function _temp2() {
        var defaults = mobxKeystone.getSnapshot(store);
        ...
        mobxKeystone.applySnapshot(store, _extends({}, defaults, state.snapshot, {
  ...
  return function (state, currentVersion) {
    try {
      ...
      if (inboundVersion === currentVersion) {
        if ("development" !== "production" && debug) console.log("mobx-keystone-persist: versions match, noop migration");
      ...
      if ("development" !== "production" && debug) console.log("mobx-keystone-persist: migrationKeys", migrationKeys);
      ...
      var _temp3 = _forOf(migrationKeys, function (versionKey) {
        ...
        if ("development" !== "production" && debug) console.log("mobx-keystone-persist: running migration for versionKey", versionKey);
...
//# sourceMappingURL=mobx-keystone-persist.cjs.development.js.map
